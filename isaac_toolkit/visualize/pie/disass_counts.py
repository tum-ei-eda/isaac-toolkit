...
def create_disass_counts_pie_plots(
    ...
    disass_instrs_hist_artifacts = filter_artifacts(
        artifacts,
        lambda x: x.flags & ArtifactFlag.TABLE and x.name == "disass_instrs_hist",
    )
    ...
    disass_opcodes_hist_artifacts = filter_artifacts(
        artifacts,
        lambda x: x.flags & ArtifactFlag.TABLE and x.name == "disass_opcodes_hist",
    )
    ...
    if disass_instrs_hist_df is not None:
        ...
        pie_plot = plot_pie_data(
            ...
            threshold=threshold,
            legend=legend,
            title="Counts (static) per Instruction",
        )
    ...
    if disass_opcodes_hist_df is not None:
        ...
        pie_plot = plot_pie_data(
            ...
            threshold=threshold,
            legend=legend,
            title="Instruction Counts (static) per Opcode",
        )
