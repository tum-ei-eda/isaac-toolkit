...
def load_mlonmcu_session_initializer(sess: Session, input_file: Path, force: bool = False):
    logger.info("Loading MLonMCU Session Initializer YAML...")
    assert input_file.is_file(), f"File not found: {input_file}"
    name = input_file.name
    attrs = {
        "by": __name__,
        "kind": "mlonmcu_session_initializer",
    }
...
def handle(args):
    ...
    load_mlonmcu_session_initializer(sess, input_file, force=args.force)
    sess.save()
