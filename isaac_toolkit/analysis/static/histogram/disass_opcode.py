...
def collect_opcodes(disass_df):
    # avoid modifying original df and copying whole df
    temp_df = disass_df[["bytecode"]].copy()
...
def create_disass_opcode_hist(sess: Session, force: bool = False):
    ...
    opcodes_df = collect_opcodes(disass_df)
    ...
    instrs_artifact = TableArtifact("disass_opcodes_hist", opcodes_df, attrs=attrs)
