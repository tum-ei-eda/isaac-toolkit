...
from isaac_toolkit.retargeting.iss.etiss_perf import retarget_etiss_perf

logger = get_logger()


def retarget_iss_perf_auto(
    sess: Session,
    label: str = "",
    ...
    if flow_config:
        ...
        if demo_config:
            sim_config = demo_config.sim
            if sim_config:
                sim_ = sim_config.iss_perf
                if sim_:
                    sim = sim_
    # input(">>>")
    assert sim == "etiss_perf"
    sim_retarget_iss = {
        "etiss_perf": retarget_etiss_perf,
    }
    ...
    if use_docker:
        docker_image = docker_config.etiss_perf_image
