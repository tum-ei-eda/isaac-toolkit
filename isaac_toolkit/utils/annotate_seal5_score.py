...
def annotate_seal5_score(index, seal5_score_csv, output=None, inplace=False):
    with open(index, "r") as f:
        combined_index_data = yaml.safe_load(f)
    candidates = combined_index_data["candidates"]
    # print("candidates", candidates)

    scores_df = pd.read_csv(seal5_score_csv)
...
def handle(args):
    annotate_seal5_score(args.index, args.seal5_score_csv, output=args.output, inplace=args.inplace)
