...
class SpikeStandaloneRunner(RISCVStandaloneRunner):
...
def invoke_spike_runner(
    ...
    # TODO: docker support
    # TODO: allow debug?
    logger.info("Running Spike program...")

    if make_target is None:
        make_target = "trace" if trace else "run"

    runner = SpikeStandaloneRunner(
        make_dir,
        isa=isa,
    )
    ...
    if label is None:
        # TODO: add timestamp?
        label = f"{program}-spike-{make_target}"
    ...
    if make_target == "run":
        ...
        if load:
            ...
            if isa:
                sim_metrics["isa"] = isa
            load_sim_metrics(sess, sim_metrics, program=program, simulator="spike", force=force)
        ...
        if load:
            # load_run_artifacts(sess, dest_dir, program, force=force)
            load_trace_artifacts(sess, dest_dir, program=program, simulator="spike", force=force)
    else:
...
def add_spike_args(parser):
    ...
    spike_group.add_argument("--isa", type=str, default=None)
