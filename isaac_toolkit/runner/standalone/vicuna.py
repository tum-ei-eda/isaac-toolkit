...
class VicunaStandaloneRunner(RISCVStandaloneRunner):
    ...
    def parse_sim_metrics(self, dest_dir, make_target: str = "run") -> dict:
        ...
        with open(outputs_file, "r") as f:
            out = f.read()
        print("out", out)
        sim_insns_match = re.search(r"Instruction Count: (\d+)", out)
        if sim_insns_match:
            sim_insns_str = sim_insns_match.group(1)
            sim_insns = int(float(sim_insns_str))
            metrics["instructions"] = sim_insns
        sim_cycles_match = re.search(r"Total Cycles: (\d+)", out)
        if sim_cycles_match:
            sim_cycles_str = sim_cycles_match.group(1)
            sim_cycles = int(float(sim_cycles_str))
            metrics["cycles"] = sim_cycles
        # cpi_match = re.search(r"CPI\s*: (\d*\.?\d*)", out)
        # if cpi_match:
        #     cpi_str = cpi_match.group(1)
        #     cpi = int(float(cpi_str))
        #     metrics["cpi"] = cpi
        if "cycles" in metrics and "instructions" in metrics:
            cpi = metrics["cycles"] / metrics["instructions"]
            metrics["cpi"] = cpi
...
def invoke_vicuna_runner(
    ...
    # TODO: docker support
    # TODO: allow debug?
    logger.info("Running Vicuna program...")

    if make_target is None:
        make_target = "trace" if trace else "run"

    runner = VicunaStandaloneRunner(
        make_dir,
        vlen=vlen,
    ...
    if label is None:
        # TODO: add timestamp?
        label = f"{program}-vicuna-{make_target}"
...
def add_vicuna_args(parser):
    ...
    vicuna_group.add_argument("--vlen", type=int, default=None)
    vicuna_group.add_argument("--vlane-width", type=int, default=None)


def parse_vicuna_args(args):
    ret = {"vlen": args.vlen, "vlane_width": args.vlane_width}
