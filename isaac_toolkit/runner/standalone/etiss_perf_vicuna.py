...
from isaac_toolkit.runner.standalone.riscv import add_riscv_args, parse_riscv_args
from isaac_toolkit.runner.standalone.etiss_perf import ETISSPerfStandaloneRunner
...
class ETISSPerfVicunaStandaloneRunner(ETISSPerfStandaloneRunner):
    ...
    def __init__(
        ...
        vlane_width: Optional[int] = None,
        **kwargs,
    ):
        super().__init__(make_dir, name, jit=jit, cpu_arch=cpu_arch, perf_model=None, **kwargs)
        if perf_model is not None:
            self.defaults["PERF_VICUNA_MODEL"] = perf_model
        if vlen is not None:
            # assert perf_model is None
            # assert cpu_arch is None
            # perf_model = f"Vicuna_zvl{vlen}b"
            # cpu_arch = f"RV32IMACFDV_zvl{vlen}b"
            # TODO: move to makefile?
            self.defaults["VLEN"] = vlen
        if vlane_width is not None:
            self.defaults["VLANE_WIDTH"] = vlane_width
...
def invoke_etiss_perf_vicuna_runner(
    sess: Session,
    make_dir: Union[str, Path],
    program: str = "unknown",
    jit: Optional[str] = None,
    ...
    runner = ETISSPerfVicunaStandaloneRunner(
        make_dir,
        jit=jit,
        cpu_arch=cpu_arch,
    ...
    if label is None:
        # TODO: add timestamp?
        label = f"{program}-etiss_perf_vicuna-{make_target}"
    ...
    if make_target == "run":
        ...
        if load:
            # load_run_artifacts(sess, dest_dir, program, force=force)
            load_trace_artifacts(sess, dest_dir, program=program, simulator="etiss_perf_vicuna", force=force)
    else:
...
def add_etiss_perf_vicuna_args(parser):
    ...
    etiss_perf_vicuna_group.add_argument("--cpu-arch", type=str, default=None)
    etiss_perf_vicuna_group.add_argument("--perf-model", type=str, default=None)
    etiss_perf_vicuna_group.add_argument("--vlen", type=int, default=None)
    etiss_perf_vicuna_group.add_argument("--vlane-width", type=int, default=None)
...
def parse_etiss_perf_vicuna_args(args):
    ret = {
        ...
        "perf_model": args.perf_model,
        "vlen": args.vlen,
        "vlane_width": args.vlane_width,
    }
